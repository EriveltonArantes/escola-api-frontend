    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Nota</li>
        </ul>
      </div>
...
function PainelProfessor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "disciplina", "titulacao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "disciplina", "titulacao"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>disciplina</b><span>{item["disciplina"] ?? "—"}</span></div>
            <div className="item-field"><b>titulacao</b><span>{item["titulacao"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["disciplina"] ?? ""} onChange={e => setForm({...form, disciplina: e.target.value})} />
      <label className="field-label">titulacao</label>
      <input type="text" value={form["titulacao"] ?? ""} onChange={e => setForm({...form, titulacao: e.target.value})} />
...
function PainelAluno({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "dataNascimento", "telefoneResponsavel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "dataNascimento", "telefoneResponsavel"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>telefoneResponsavel</b><span>{item["telefoneResponsavel"] ?? "—"}</span></div>
            <div className="item-field"><b>turma</b><span>{item.turmaId ? ((turmaList.find(o => o.id === item.turmaId) || {}).nome ?? ("#" + item.turmaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      <label className="field-label">telefoneResponsavel</label>
      <input type="text" maxLength={15} value={form["telefoneResponsavel"] ?? ""} onChange={e => setForm({...form, telefoneResponsavel: formatarTelefone(e.target.value)})} />
      <label className="field-label">turma</label>
      <select value={form["turma"] ?? ""} onChange={e => setForm({...form, turma: e.target.value})}>
        <option value="">Selecione...</option>
        {(turmaList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelTurma({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "serie", "turno", "ano", "professorId", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "serie", "turno", "ano", "professorId"].forEach(k => { f[k] = item[k] ?? ""; });
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Turma")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>serie</b><span>{item["serie"] ?? "—"}</span></div>
...
function PainelNota({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "turmaId", "bimestre", "notaAvaliacao", "notaTrabalho", "media", "situacao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/notas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/notas/" + editId : apiBaseUrl + "/api/notas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "turmaId", "bimestre", "notaAvaliacao", "notaTrabalho", "media", "situacao"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "turma"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/notas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
    ...
    <div>
      ...
      {modoKanban && (
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.situacao ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              <div className="kanban-coluna-titulo">
                ...
                <span className="kanban-coluna-contagem">{itensFiltrados.filter(i => String(i.situacao ?? "Sem status") === coluna).length}</span>
              </div>
              {itensFiltrados.filter(i => String(i.situacao ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["situacao"] ?? "Nota")}</div>
                  <select className="kanban-select" value={item.situacao ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/notas/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, situacao: e.target.value }),
                    }).then(carregar);
                  }}>
                    {[...new Set(itensFiltrados.map(i => String(i.situacao ?? "Sem status")))].map(s => (<option key={s} value={s}>{s}</option>))}
                  </select>
                </div>
              ))}
            </div>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["situacao"] ?? "Nota")}</div>
            <span className={"status-pill " + corStatus(item.situacao)}>{String(item.situacao ?? "")}</span>
            <div className="item-meta-grid">
            ...
            <div className="item-field"><b>bimestre</b><span>{item["bimestre"] ?? "—"}</span></div>
            <div className="item-field"><b>notaAvaliacao</b><span>{item["notaAvaliacao"] != null ? Number(item["notaAvaliacao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>notaTrabalho</b><span>{item["notaTrabalho"] != null ? Number(item["notaTrabalho"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>media</b><span>{item["media"] != null ? Number(item["media"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>aluno</b><span>{item.alunoId ? ((alunoList.find(o => o.id === item.alunoId) || {}).nome ?? ("#" + item.alunoId)) : "—"}</span></div>
            <div className="item-field"><b>turma</b><span>{item.turmaId ? ((turmaList.find(o => o.id === item.turmaId) || {}).nome ?? ("#" + item.turmaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
      ...
      <input type="number" value={form["bimestre"] ?? ""} onChange={e => setForm({...form, bimestre: e.target.value})} />
      <label className="field-label">notaAvaliacao</label>
      <input type="number" step="0.01" value={form["notaAvaliacao"] ?? ""} onChange={e => setForm({...form, notaAvaliacao: e.target.value})} />
      <label className="field-label">notaTrabalho</label>
      <input type="number" step="0.01" value={form["notaTrabalho"] ?? ""} onChange={e => setForm({...form, notaTrabalho: e.target.value})} />
      <label className="field-label">media</label>
      <input type="number" step="0.01" value={form["media"] ?? ""} onChange={e => setForm({...form, media: e.target.value})} />
      <label className="field-label">situacao</label>
      <select value={form["situacao"] ?? "ATIVO"} onChange={e => setForm({...form, situacao: e.target.value})}>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Escola<span className="dot">.</span></div>
        ...
        <button className={"nav-btn" + (aba === "Nota" ? " active" : "")} onClick={() => setAba("Nota")}><span className="nav-ico">🗂️</span>Nota</button>
      ...
      {aba === "Nota" && <PainelNota token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
